...
| --- | --- | --- |
| 2026-08-30 | Negotiated Cargo Exchange offers show the universal catalog-based price distribution (minimum, Q1, median, Q3, maximum) with the actual quote marked. Purchase landmarks span 80%-120% of catalog; sale landmarks span 80%-156% by combining every possible original purchase basis and buyer markup. They are independent of the selected negotiator and appear only after negotiation. CT-RPC 12 adds the distribution to `MarketLead`, and retained-outcome codec 32 preserves it while codec 31 defaults it to zero. | Current |
| 2026-08-30 | Port-research assignments remain visible as history after completion, failure, or cancellation until the captain explicitly dismisses one. Dismissal permanently deletes only the finished assignment record, never its resulting counterparty lead; scheduled work must use cancellation first. The additive command remains within the current unreleased CT-RPC 12 contract. | Current |
| 2026-08-30 | Docked Operations shows the server-calculated outstanding origin export tariff separately from the accrued berth fee; both are operating-account charges collected when the ship clears its berth. CT-RPC 12 adds this value to `DockedSnapshot`, and retained-outcome codec 31 preserves it while older snapshots decode it as zero. | Current |
| 2026-08-28 | The universal Command Console presents four focused areas: Vessel and Crew; Duty and Accounts; Information and Communications; and Captain and Interface. The eight established manager letters remain direct shortcuts from the root for experienced players. `Q` backs from an area to the root, root `Q` alone offers BBS exit, and `X` returns directly to the operational view. Each area owns beginner and expert contextual help, and any later menu membership or behavior change must review that area's help. | Current |
| 2026-08-28 | Accounts is a universal read-only manager, distinct from dock-only Banking Services. It shows liquid, available-after-reservations, reserved, restricted operating, and principal balances; certified task income is grouped by estimated remittance-resolution day and is never counted as cash before physical receipt. The durable newest-first account journal records typed opening, income, expense, transfer, hold, and financing entries with per-account amount and resulting balance in the same transaction as each balance mutation; existing format-2 estates receive one idempotent carried-forward opening entry. Class and vessel filters are server-side and history is retained indefinitely. `ServerHello.accountJournalAvailable` gates the additive CT-RPC 10 query for compatibility with earlier CT-RPC 10 servers. Any future menu or field change must review both beginner and expert help for that surface; this feature adds explanatory help rather than regex-based CI copy tests. | Current |
| 2026-08-27 | Jump traffic uses separate conventional loci on opposite sides of the system ecliptic: departures north and primary-world arrivals south. A charted leg selects its target system, first named in-system destination, and then standard or seeded private/offset emergence; standard and private body-targeted points are outside every 100-diameter exclusion zone and are durably frozen when Jump begins. Breakout is followed by bounded-thrust interplanetary travel to the selected port, world/moon, gas giant, or belt, after which further local stops or another Jump may follow. Either conventional primary-world locus remains a legal maneuver destination and interception-watch station; using the wrong published lane is allowed with a traffic warning. Private emergence skips the ordinary arrival-locus contact but adds standoff distance; later port contact and high-law mandatory inspection still apply at docking. Onboard fuel processing accepts 0.001-ton batches and remains a stationary Flight Plan action. CT-RPC 10 ships with v0.7.14; Flight Plan codec 6, ship codec 5, interception-watch codec 3, and retained-outcome codec 29 persist the state. | Current |
| 2026-08-27 | Enhanced-profile option menus use normal green for shortcut brackets, reserve bright-green Prompt for the key text inside and the final input marker, and use grey Muted text for action descriptions; free-form input prompts remain wholly bright green and plain profiles remain byte-for-byte visually unchanged. This is a shared presentation rule for every structured option or choice prompt, not a screen-specific palette. | Current |
| 2026-08-27 | A cooperative encounter contact transmits a declared ship name, transponder, and registered class as claims distinct from the observing ship's sensor result. Arrival and Command Loss views label those claims as Declared identity/class and label the existing generic size estimate and confidence as Sensor classification; dark or non-identifying contacts omit declared class. CT-RPC 10 adds the field additively, encounter codec 5 and retained-outcome codec 24 persist it, and legacy readers supply an empty declaration. | Current |
| 2026-08-27 | Every arrival encounter offers Fight against its displayed contact; crew on watch are assumed close enough to battle stations that no separate pre-combat readiness state is needed. Fight enters the existing combat path instead of completing a routine or non-hostile response. Unless an accepted Naval Order or Privateer Commission names that exact contact, initiating combat against a contact that has not attacked adds ordinary armed-interception heat and a physical-mail warrant and changes an Independent force-career record to Pirate. Existing Through-point policy never chooses this new escalation automatically. | Current |
| 2026-08-26 | Encounter choices use contact-specific operational verbs rather than exposing the generic stored posture vocabulary. Routine traffic is an informational exchange acknowledged with Enter and resolves in the response transaction without a scheduled combat turn. Inspection, traffic-control, distress, derelict, hazard, military, hostile, and departing-contact screens name the actual action; any displayed wait is authoritative action time, never approval by the other vessel. Existing typed postures remain the CT-RPC compatibility representation. | Current |
...
| 2026-07-31 | The initial authoritative simulation/mail substrate is implemented. `SystemDay` deterministically generates physical commercial-offer instruments and ordinary departures from the persisted system seed and logical day; it does not manufacture generic market, public-information, traffic, or recipientless-private filler. Meaningful event- and player-authored messages fan out into route-specific envelopes; beacon queues load up to 512 eligible envelopes into a sealed mailbag; a named simulated traffic ship and custody leg carry it over a separately scheduled one-week Jump; arrival delivers, forwards, or expires each envelope. Every event currently remains an individual ordered LMDB transaction. The tour audits custody and reopen recovery and reports unique/copy message volume, thread CPU per system, and observed whole-universe progression rates. Common Goods cargo, player itineraries, player mail custody, and live clock advancement now exist; passengers and presentation of the player circuit in the non-interactive tour remain pending. Full boundary and calibration are in `docs/non-interactive-universe-tour.md`. | Current |
| 2026-08-26 | Message Management and arrival review share the captain's four visible communication thresholds: agency news, public service, traffic, and private. Raising a threshold hides lower-priority retained copy from both surfaces and lowering it reveals that copy again. Commercial offers remain physical delayed instruments but appear only in Task Management; their legacy communication filing rows are removed. Existing exact generated filler is purged atomically from messages, envelopes, queues, deliveries, mailbags, and player filing state while custody history and already-advertised carrier stipends remain valid. | Current |
...
| 2026-07-31 | Capacity baselines may deterministically provision multiple configured BBS polities and report LMDB used bytes in addition to event and CPU rates. The ten-BBS settlement-edge fixture fixes `E = 28.993534 pc`, resolves the full `3E = 86.980601 pc` sphere with the Galactic inhomogeneous Poisson sampler, and contains 238,812 systems in 77,991,936 initial LMDB bytes. A 10,000-event day-zero prefix retained 5,914,624 bytes and projected at least 87.4 million system-day transactions plus enough departures/arrivals for roughly 752 million total events and 34.5 wall-days for one year under the then-current individual-commit and filler-message model. These are measured historical projections, not current message-volume calibration. Retained meaningful messages still require storage growth to be reported at an age and per game day/year rather than as a timeless maximum. Full results are in `docs/non-interactive-universe-tour.md`. | Current |
...
| 2026-07-29 | `Docked` is derived from a concrete ship attachment to a body and facility, including landed/orbital state, berth where relevant, arrival time, fees, and locally available services. The phase menu provides Cargo Exchange, Jobs and Passage, Fuel and Supplies, Shipyard, Personnel, Banking Services, Authorities, and Depart. Universal managers, including read-only Accounts, are linked rather than duplicated. Implementation order is location, facility snapshot, menu, fuel/supplies/departure, proper repair/maintenance, then the larger economy. Full design is in `docs/docked-operations.md`. | Current |
...
| 2026-07-29 | The OpenDoors client implements a Docked Operations menu with `C/J/F/Y/P/B/A/D` for Cargo Exchange, Jobs and Passage, Fuel and Supplies, Shipyard, Personnel, Banking Services, Authorities, and Depart. `U` enters the universal-manager shell; that shell includes `K` for Known Universe and `F` for Accounts. Every offered action uses server-supplied state. Persistent facility capabilities control fuel, chandlery, ordnance, yard, medical, personnel, banking, and authority availability in both quotation and commit; absent services are hidden and remain authoritatively rejected. | Current |
...
| 2026-08-26 | Proper subsystem repair is berth-bound but interruptible. A Flight Plan that will clear the berth warns on the first departure step that clears the berth and requires acknowledgement; successful departure atomically removes the repair activity and scheduled completion while leaving damage unchanged. Refit, refurbishment, construction, fuel processing, and other active work remain departure blockers. Ship status exposes the authoritative clock ratio so scheduled-work time can show both game duration and equivalent wall time. These are additive CT-RPC 10 fields in the current unreleased line and do not independently require another version bump. | Current |
| 2026-08-01 | The authoritative market implements all six Common Goods and 35 generic revised *Bounded Fortune* trade goods without optional PI examples. Finite system/day stock and shared consumption are durable; trade-code, Broker/Charisma, legality, and generated tariff effects are applied. Purchase outcomes are 80/90/100/120 percent and sale markups are 30/15/2/0 percent. Ordinary negotiations hold for seven game days from port arrival or a newly acquired lot's acquisition; daily stock never rerolls price, and ordinary origin-market bids remain below the lot's actual purchase price. Physical/private/hired searches take 1D6 days, online searches 1D6 hours, and prior attempts in the same 30-day system market apply cumulative DM-1. Private contraband searches use Streetwise/Charisma to find a counterparty and Broker/Charisma to negotiate. Supplier/buyer research is timed queued work which produces dated confidence ranges in Known Universe. Exact milliton cargo and whole-credit rounding remain authoritative. | Superseded by the 2026-08-30 Bounded Fortune counterparty workflow below. |
| 2026-08-13 | Cargo Exchange buy and sell rows show an absolute universe-wide commodity market-value span (minimum, Q1, median, Q3, maximum) with the captain-specific current quote marked. The span is not conditioned on the current captain; help states that Broker, Charisma, local trade codes, events, tariffs, and the ordinary spread can change the actual quote. Purchase favorability runs low-to-high while sales reverse it, and every green/yellow/red band also has a textual price judgment. Mean is intentionally omitted. | Superseded by the 2026-08-30 catalog-reference and negotiated-quote display. |
| 2026-08-03 | Merchant research produces finite, expiring, revisioned leads that can be performed while available; optional reservation escrows ten percent and does not refund an expired or released opportunity payment. Named market events persist exact stock/price effects and causal agency news. Ordinary freight, passage, and mail use a previewed atomic manifest for the committed destination. Remote offer claims and their replies physically race through mail; no custody transfers until the captain receives the award. Task withdrawal, cancellation, custody return, capped default, and sealed dispute filing are revision-checked transactions; disputed obligations retain collateral and capacity. Fixed-system and mobile-captain private mail is physical and priced at Cr1 per started KiB per charged hop per started TTL week within a one-to-52-week TTL. Destination assistance is an optional Cr350,000 annual dockside policy without cancellation refund. | Market reservation/event clauses superseded by the 2026-08-30 workflow; the remaining clauses stay current. |
...
| 2026-08-29 | Routine-upkeep payment is all-or-nothing. If restricted operating credit plus liquid cash cannot cover the complete monthly charge, neither balance is partially consumed; the cycle is recorded as missed and its neglect check resolves immediately. There is no monetary maintenance-arrears liability to cure later. Consecutive missed upkeep cycles drive neglect risk and reset after a fully funded cycle, while existing damage remains. Storage-format-2 and CT-RPC retain their retired arrears slots at zero for compatibility. | Current |
...
- Speculative trade follows the Bounded Fortune/Clement procedure: timed
  supplier/buyer search produces finite source-sized counterparties, then a
  separate timed Broker+CHA negotiation produces an executable quote, and only
  explicit acceptance transfers the complete lot. There are no anonymous free
  quotes, direct spot trades, reservations, or market-event price modifiers.
  Preserve same-port resale prevention. Sales use actual purchase/appraisal
  basis; purchases add automatic 10% loaders; buyer bears import tariff and the
  merchant pays excluded origin export tariff when clearing the berth. Until
  the setting-neutral captains association is implemented, all captains receive
  its Average ordinary-trade difficulty benefit; prohibited trade does not.
