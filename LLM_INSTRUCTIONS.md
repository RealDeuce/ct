...
| 2026-08-03 | Player RPCs are exhaustively classified as observations or transactions. Both remain ordered and journaled; only transactions retain exactly-once command results and full journal deliveries, while observation journals retain the request without an indefinitely stored snapshot. Player CT-RPC version 2 and storage format 1 accept only current shapes; the undeployed server performs no compatibility migrations or startup index reconstruction. See `docs/rpc-and-storage-schema.md`. | Current |
