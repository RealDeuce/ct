...
constexpr std::array<DoorHelp, static_cast<size_t>(DoorHelpTopic::Count)> HELP{{
   ...
   {"Door controls and paging", "Getting started", Category::GettingStarted,
    "The options printed at the bottom of a screen are the controls for that screen. Letters are not case-sensitive. Enter usually accepts the displayed default, refreshes current information, or returns to the previous view. Q usually cancels or goes back; leaving the game requires confirmation.\n\nLong output pauses before text can scroll away. Enter or Space advances one page. C makes the rest of that output continuous until the next keyboard input. Player Preferences can durably disable these automatic pauses while leaving choices and confirmations intact. In help, B restarts the topic with beginner explanations, X restarts it in expert mode, and Q stops help immediately.\n\nAngle-bracket keys move between list pages where shown. Some screens also accept arrow keys, but printable alternatives are always provided for terminals that cannot send arrow sequences.",
    "Options are case-insensitive. Enter accepts, refreshes, or returns as displayed; Q cancels or backs out. Pager controls are Enter/Space for one page and C for continuous output. Automatic page pauses can be disabled durably in Player Preferences. Help additionally uses B/X to restart in beginner/expert mode and Q to stop. Printable navigation alternatives accompany terminal-specific keys."},

   {"A useful first watch", "Getting started", Category::GettingStarted,
    "After registration your ship begins docked. Start by opening the Command Console and looking at Crew, Ship Management, and the command's accounts. Notice crew coverage, fuel, provisions, damage, endurance, title, and the money or issued funds the command may actually use.\n\nThe next duty depends on the command. A trader inspects Tasks and commercial capacity. A privateer reviews Operations and the commission before considering any ordinary work the command is eligible to take. A naval captain reviews issued orders in Operations and does not need to visit commercial cargo, passenger, or offer screens. Every captain should read Messages, then compare the duty or chosen destination with Known Universe before plotting a course.\n\nThere is no prize for launching immediately. A well-crewed, fuelled ship with a feasible plan is safer than a grand vessel sent out on an assumption.",
    "Inspect Crew, Ship, accounts, Messages, and Known Universe before departure. Traders review commercial Tasks; privateers review Operations before optional eligible work; naval captains follow issued orders instead of commercial offers. Verify duty coverage, fuel, provisions, authority, destination knowledge, and timing. Plot, preview, then file an executable flight plan."},

   {"Guided First Watch", "Getting started", Category::GettingStarted,
    "Guided First Watch is the command library's optional tour of your real captain, crew, ship, accounts, information, duty, charts, and departure procedure. Its route follows the command: traders inspect commercial Tasks, privateers review their commission before optional eligible work, and naval captains follow Operations and issued orders without being sent through commercial offer lessons. It does not create a practice ship, stop time, change prices, reserve an offer, accept work, or relax any rule. Other captains and the universe continue normally.\n\nEach briefing opens the same ordinary manager available from the Command Console. S skips one explanation, Q leaves progress available for later, and H hides guidance. Player Preferences can begin, resume, hide, or restart the explanations without rewinding anything that happened in the game. First Watch ends only after the server accepts a real Flight Plan and the ship departs. A preview can always be revised or abandoned without filing it.",
    "Optional client guidance over ordinary live managers. Enter opens the named screen; S advances past presentation copy; Q suspends; H hides. Progress is a BBS-local presentation preference. No offer is accepted and no game state changes except through the ordinary screen's explicit actions. Completion follows a successful Flight Plan commit, never inferred from travel phase."},
   ...
   {"Player preferences", "Getting started", Category::GettingStarted,
    "Player Preferences stores presentation choices for this local BBS identity on later calls. The default help level controls what appears when you press ?. Beginner mode explains the game idea behind a screen; Expert mode concentrates on fields, actions, and consequences.\n\nAutomatic page pauses keep long output from scrolling past. You may durably disable them when your terminal or BBS keeps a usable scrollback. This does not skip menus, decisions, confirmations, or other input the game actually requires. First Watch controls begin, resume, hide, or restart its explanations. Restarting clears only which briefings this client remembers; it does not rewind the captain or universe. None of these preferences changes difficulty or permissions.",
    "Set the durable Beginner/Expert help default, automatic continuation pauses, and local First Watch presentation state. Restarting First Watch clears presentation acknowledgements only. These settings have no gameplay effect."},
   ...
   {"Captain's Command Console", "Universal managers", Category::MenusScreens,
    "The Command Console is the doorway to information and management that remains available while docked or travelling. Crew covers people; Ship covers vessels and stores; Tasks covers opportunities and obligations; Messages covers delayed electronic correspondence; Known Universe covers charts and routes; Operations covers traffic, authority, warrants, and fighting; System Common is public local radio.\n\nW opens Guided First Watch while docked, H opens the full help browser, and P changes player preferences. X returns to the operational view. Q asks before leaving the game. Permanently abandoning the captain is deliberately separate and requires an exact phrase plus another confirmation.",
    "Open any universal manager, Guided First Watch while docked, the help browser, or player preferences. X returns to the operational screen; Q confirms return to the BBS. Abandon Captain permanently deletes the complete estate after two confirmations."},

   {"Ship management", "Universal managers", Category::MenusScreens,
    "A vessel is a physical asset with an owner or issuing authority, a current commander, a location, installed systems, fuel, stores, cargo, damage, and crew requirements. Ship Management gathers those facts and lets a captain manage every vessel in the estate.\n\nDo not confuse being assigned a ship with owning it. Transfers change real custody. Damage can disable a system temporarily or require proper yard repair. Fuel, ammunition, provisions, and cargo all occupy or depend on finite capacity.\n\nRoutine upkeep is continuous shipboard work. Its cost is charged automatically every 30 game days, so no monthly yard order is needed. The operating account uses restricted operating credit first, then liquid credits. If it cannot cover the full charge, the shortfall becomes arrears and the missed cycle can damage a subsystem. Proper repairs and refits are separate yard work; a refit does not suspend routine upkeep.",
    "Review title, command, location, activity, capacity, stores, complement, installed systems, damage, upkeep, and fleet custody. Routine upkeep is automatic every 30 game days, draws restricted operating credit before liquid credits, and can cause subsystem damage when underfunded. Proper repair and refit are separate yard work. Actions apply to the selected physical vessel."},
   ...
   {"Course plotter", "Universal managers", Category::MenusScreens,
    "The course plotter searches known space for an executable chain of jumps and fuel stops. Jump rating limits each jump, not the total trip. The search tracks current fuel, tank size, known port sales, lawful gas-giant or wilderness collection, money, and elapsed time.\n\nFastest and cheapest can choose different paths. The plotter and Flight Plan can also suggest one route covering every active accepted obligation assigned to this ship. That suggestion uses a bounded, deadline-aware heuristic instead of trying every possible stop order, so it returns quickly but is not guaranteed to be the mathematical optimum. Required pickups precede their deliveries. Always preview the imported plan: the preview is the authoritative deadline check. Importing edits the flight plan; it does not launch the ship.",
    "Plot fastest or cheapest executable courses, or quickly suggest a precedence- and deadline-aware route through all active task stops assigned to this ship. The bounded heuristic is not guaranteed optimal. Preview before filing."},
   ...
   {"Cargo exchange", "Dock services", Category::MenusScreens,
    "Speculative cargo is merchandise you own and hope to resell. Entrusted freight belongs to a charter or other principal and normally cannot be sold. The exchange's buy price is what you pay to load; the sell price is the actual local bid for cargo aboard. The ordinary bid stays below the ordinary ask, so unloading immediately is not free profit.\n\nEach price plot compares your current quote with the universe-wide market-value range for that commodity: minimum, lower quartile, median, upper quartile, and maximum. That reference is not tailored to your captain. The current quote is captain-specific: Broker skill, Charisma, local trade conditions, market events, and tariffs can all affect what this captain actually gets. Green marks the favorable whisker, yellow the adjacent quartile, and red a median-or-worse quote; selling reverses the scale. The words beside the quote carry the same meaning without color.\n\nCapacity, lot size, legality, market depth, brokerage, and cash constrain trades. Prices and reports are local and dated. Review tonnes, unit price, total, ownership, and remaining hold before confirming.",
    "Buy owned speculative lots or sell eligible cargo at the exact local bid. Price plots use universe-wide market values; only the current quote reflects Broker, Charisma, local trade conditions, events, and tariffs. Green is favorable, yellow middling, and red median-or-worse, reversed for sales and repeated in text. Hold capacity, lot identity, ownership, legality, depth, cash, and dated observations apply."},
   ...
   {"Banking and accounts", "Dock services", Category::MenusScreens,
    "Accounts distinguish spendable personal cash from debts, reserved collateral, arrears, insurance, assistance, and restricted operating credit. A privateer may command a sponsor-owned vessel with no purchase loan but still owe its mandatory monthly insurance. Restricted operating credit pays that insurance and other authorized ship expenses before personal cash; it cannot pay secured principal or private purchases.\n\nIf an installment defaults, choose P to post the displayed overdue amount and withdraw the active impound order. This is available from an impound message even while the ship is in Jump. Records at remote offices can still lag behind events travelling by mail. Naval captains can forge an expense receipt, but later audits can create service and legal consequences.",
    "Inspect title, liquid cash, secured principal, mandatory insurance, arrears, assistance, and restricted operating credit. Authorized insurance/ship costs draw restricted funds first; secured principal requires liquid cash. P cures one overdue installment and withdraws its impound order from any phase. Enforcement knowledge remains subject to mail delay; forged naval conversion risks audit findings."},

   {"Shipyard and repairs", "Dock services", Category::MenusScreens,
    "Shipyards sell and trade vessels, replace machinery, refit systems, and repair lasting damage--but only within local technical and industrial ability. The local Tech Level, port, and actual facility are separate limits. Cepheus Trader's vessel catalog begins at TL9, so a lower-TL port can legitimately have no ships for sale. Quotes depend on this exact hull, its title, damage, required parts, and time.\n\nA disabled system recovered after battle is not necessarily repaired. Trade-in transfers a real vessel with whatever stores, custody, and obligations the displayed terms include.",
    "Review vessel sales, trade-ins, component replacement, refits, and proper repairs available at this yard. World TL, port and facility capability, parts, title, damage, quote, and work time constrain orders. The current vessel catalog begins at TL9; a lower-TL market can be empty."},
   ...
   {"Tech levels and local capability", "Worlds and ports", Category::Concepts,
    "Tech Level, or TL, measures a world's scientific and production capability and the complexity it can manufacture locally. It is not the same as population, wealth, law, port class, or the capability of one particular facility. Imports can exceed local TL, while a poor port or closed yard can prevent work that the world's TL would otherwise permit.\n\nThe broad scale is:\nTL0: no technological base.\nTL1-3: primitive through early mass production and steam.\nTL4-6: industrialization, radio, electrification, combustion, fission, and increasingly capable computers.\nTL7-8: reliable orbital and then practical interplanetary travel.\nTL9: gravity technology and the first steps toward Jump. This is the lowest TL in Cepheus Trader's current vessel catalog.\nTL10-11: early interstellar technology and increasingly autonomous computers.\nTL12-14: mature interstellar technology, including advanced planetary engineering, armor, computers, and weapons.\nTL15 and higher: high stellar technology.\n\nThese examples explain the scale; they do not promise that every named technology is implemented as a game system. In play, compare TL with the actual port and facility. A world below TL9 can have a working port but no locally eligible ships for sale.",
    "TL is the world's local scientific/manufacturing capability, distinct from population, law, port class, and facility capability. TL0-3 is primitive; TL4-6 industrial; TL7-9 orbital/pre-stellar; TL10-11 early interstellar; TL12-14 mature interstellar; TL15+ high stellar. Imports may exceed local TL. The current vessel catalog begins at TL9, and port/facility limits still apply independently."},
