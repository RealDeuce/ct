...
class CatalogRecordTests(unittest.TestCase):
    ...
    def test_marathon_exposes_assault_cruiser_fit(self) -> None:
        marathon = self.records[103]
        self.assertEqual(marathon["family_id"], 103)
        self.assertEqual(marathon["path_name"], "Redoubt")
        self.assertEqual(marathon["tons"], 1200)
        self.assertEqual(marathon["configuration"], "Standard")
        self.assertEqual(marathon["tech_level"], 11)
        self.assertFalse(marathon["standard_design"])
        self.assertEqual(marathon["electronics"], "Advanced")
        self.assertEqual(marathon["armor_points"], 8)
        self.assertEqual(marathon["bridge_options"], ["Hardened Bridge"])
        self.assertEqual(marathon["computer"], "Model 3")
        self.assertEqual(marathon["computer_options"], ["Bis", "Fib"])
        self.assertEqual(marathon["jump_drive"], "L")
        self.assertEqual(marathon["jump_distance"], 2)
        self.assertEqual(marathon["jump_count"], 1)
        self.assertEqual(marathon["maneuver_drive"], "S")
        self.assertEqual(marathon["power_plant"], "S")
        self.assertEqual(marathon["endurance"], 3)
        self.assertEqual(marathon["cargo"], "193.9 tons")
        self.assertEqual(marathon["crew"], 36)
        self.assertIn("30 × Barracks", marathon["equipment"])
        self.assertIn("3 × Armory", marathon["equipment"])
        self.assertIn("5 × Fuel Processor", marathon["equipment"])
        self.assertIn("Fuel Scoop", marathon["equipment"])
        self.assertIn("3 × Atv Hangar", marathon["equipment"])
        self.assertIn("Full Hangar (110 tons contained)", marathon["equipment"])
        self.assertIn("Carried Craft: Proteus Cargo (ship-20)", marathon["equipment"])
        self.assertIn("2 × Carried Craft: Jason (ship-17)", marathon["equipment"])
        self.assertIn("4 × Triple Turret: Beam Laser · Beam Laser · Beam Laser", marathon["armament"])
        self.assertIn("2 × Triple Turret: Missile Rack · Missile Rack · Missile Rack", marathon["armament"])
        self.assertIn("2 × Triple Turret: Sandcaster · Sandcaster · Sandcaster", marathon["armament"])
        self.assertIn("4 × Particle Beam Barbette", marathon["armament"])
        self.assertIn("12 × Point Defense Node Mount: Point Defense Laser", marathon["armament"])
        self.assertEqual(marathon["ammunition"], "120 × Standard Missiles · 120 × Sandcaster Canisters")
        self.assertEqual(marathon["unused_fire_control_stations"], 0)
    ...
    def test_hannibal_exposes_assault_carrier_fit(self) -> None:
        ship = self.records[113]
        self.assertEqual(ship["family_id"], 113)
        self.assertEqual(ship["path_name"], "Admiralty Line")
        self.assertEqual((ship["tons"], ship["configuration"]), (2500, "Streamlined"))
        self.assertEqual((ship["tech_level"], ship["armor_points"]), (11, 4))
        self.assertFalse(ship["standard_design"])
        self.assertEqual(ship["electronics"], "Advanced")
        self.assertEqual(ship["bridge_options"], ["Command Bridge", "Hardened Bridge", "Holographic Controls"])
        self.assertEqual(ship["computer_options"], ["Bis", "Fib"])
        self.assertEqual((ship["jump_drive"], ship["jump_distance"]), ("T", 2))
        self.assertEqual((ship["maneuver_drive"], ship["power_plant"]), ("Z", "Z"))
        self.assertEqual((ship["endurance"], ship["cargo"], ship["crew"]), (3, "4 tons", 149))
        self.assertIn("200 × Barracks", ship["equipment"])
        self.assertIn("20 × Armory", ship["equipment"])
        self.assertIn("20 × Emergency Low Berth", ship["equipment"])
        self.assertIn("5 × Atv Hangar", ship["equipment"])
        self.assertIn("Medical Ward (20 beds)", ship["equipment"])
        self.assertIn("Full Hangar (100 tons contained)", ship["equipment"])
        self.assertIn("Full Hangar (210 tons contained)", ship["equipment"])
        self.assertIn("2 × Flight Deck (10 tons largest craft)", ship["equipment"])
        self.assertIn("10 × Carried Craft: Icarus II (ship-11)", ship["equipment"])
        self.assertIn("2 × Carried Craft: Goliath (ship-30)", ship["equipment"])
        self.assertIn("Carried Craft: Jason (ship-17)", ship["equipment"])
        self.assertIn("13 × Triple Turret: Beam Laser", ship["armament"])
        self.assertIn("5 × Triple Turret: Missile Rack", ship["armament"])
        self.assertIn("5 × Triple Turret: Sandcaster", ship["armament"])
        self.assertIn("2 × Particle Beam Bay", ship["armament"])
        self.assertIn("25 × Point Defense Node Mount", ship["armament"])
        self.assertEqual(ship["ammunition"], "240 × Standard Missiles · 300 × Sandcaster Canisters")
        self.assertEqual(ship["magazine_options"], ["Improved Magazine"])
        self.assertEqual(ship["unused_fire_control_stations"], 0)
        self.assertEqual(ship["length_m"], 122.0)
        self.assertEqual(ship["art_path"], "assets/ships/ship-113-hannibal.webp")

    def test_tsushima_exposes_flag_fleet_cruiser_fit(self) -> None:
        ship = self.records[114]
        self.assertEqual(ship["family_id"], 114)
        self.assertEqual(ship["path_name"], "Admiralty Line")
        self.assertEqual((ship["tons"], ship["configuration"]), (2000, "Standard"))
        self.assertEqual((ship["tech_level"], ship["armor_points"]), (11, 10))
        self.assertFalse(ship["standard_design"])
        self.assertEqual(ship["electronics"], "Advanced")
        self.assertEqual(ship["bridge_options"], ["Command Bridge", "Hardened Bridge", "Holographic Controls"])
        self.assertEqual(ship["computer_options"], ["Bis", "Fib"])
        self.assertEqual((ship["jump_drive"], ship["jump_distance"]), ("Q", 2))
        self.assertEqual((ship["maneuver_drive"], ship["power_plant"]), ("Y", "Y"))
        self.assertEqual(ship["assertions"]["thrust_g"], 4)
        self.assertEqual((ship["endurance"], ship["cargo"], ship["crew"]), (3, "127.666 tons", 124))
        self.assertIn("14 × Fuel Processor", ship["equipment"])
        self.assertIn("Medical Bay (3 beds)", ship["equipment"])
        self.assertIn("Full Hangar (50 tons contained)", ship["equipment"])
        self.assertIn("Carried Craft: Argo (ship-10)", ship["equipment"])
        self.assertIn("Carried Craft: Wayfarer Cargo (ship-187)", ship["equipment"])
        self.assertIn("3 × Triple Turret: Beam Laser", ship["armament"])
        self.assertIn("3 × Triple Turret: Missile Rack", ship["armament"])
        self.assertIn("2 × Triple Turret: Sandcaster", ship["armament"])
        self.assertIn("4 × Meson Gun Bay", ship["armament"])
        self.assertIn("4 × Particle Beam Bay", ship["armament"])
        self.assertIn("20 × Point Defense Node Mount", ship["armament"])
        self.assertEqual(ship["ammunition"], "244 × Standard Missiles · 160 × Sandcaster Canisters")
        self.assertEqual(ship["structural_options"], ["Reinforced Structure (1 increment)", "Armored Bulkheads (Bridge)", "Armored Bulkheads (Drives)"])
        self.assertEqual(ship["magazine_options"], ["Improved Magazine"])
        self.assertEqual(ship["unused_fire_control_stations"], 0)
        self.assertEqual(ship["length_m"], 108.0)
        self.assertEqual(ship["art_path"], "assets/ships/ship-114-tsushima.webp")

    def test_midway_exposes_loaded_gunboat_carrier_fit(self) -> None:
        ship = self.records[115]
        self.assertEqual(ship["family_id"], 115)
        self.assertEqual(ship["path_name"], "Admiralty Line")
        self.assertEqual((ship["tons"], ship["configuration"]), (2000, "Close Structure"))
        self.assertEqual((ship["tech_level"], ship["armor_points"]), (10, 2))
        self.assertFalse(ship["standard_design"])
        self.assertEqual(ship["electronics"], "Basic Military")
        self.assertEqual(ship["hull_options"], ["Radiation Shielding", "Self Sealing"])
        self.assertEqual(ship["bridge_options"], ["Command Bridge", "Hardened Bridge", "Holographic Controls"])
        self.assertEqual(ship["computer_options"], ["Fib"])
        self.assertEqual((ship["jump_drive"], ship["jump_distance"]), ("Y", 2))
        self.assertEqual((ship["maneuver_drive"], ship["power_plant"]), ("Y", "Y"))
        self.assertEqual(ship["assertions"]["effective_displacement_millitons"], 3800000)
        self.assertEqual(ship["assertions"]["thrust_g"], 2)
        self.assertEqual((ship["endurance"], ship["cargo"], ship["crew"]), (2, "188.75 tons", 151))
        self.assertIn("15 × Fuel Processor", ship["equipment"])
        self.assertIn("Full Hangar (80 tons contained)", ship["equipment"])
        self.assertIn("6 × Docking Clamp 300", ship["equipment"])
        self.assertIn("6 × Carried Craft: Monitor (ship-70)", ship["equipment"])
        self.assertIn("2 × Carried Craft: Archimedes (ship-124)", ship["equipment"])
        self.assertIn("2 × Carried Craft: Wayfarer Cargo (ship-187)", ship["equipment"])
        self.assertIn("5 × Triple Turret: Turret Railgun", ship["armament"])
        self.assertIn("5 × Triple Turret: Beam Laser", ship["armament"])
        self.assertIn("5 × Triple Turret: Missile Rack", ship["armament"])
        self.assertIn("5 × Triple Turret: Sandcaster", ship["armament"])
        self.assertIn("10 × Point Defense Node Mount: Point Defense Minigun", ship["armament"])
        self.assertIn("10 × Point Defense Node Mount: Point Defense Laser", ship["armament"])
        self.assertEqual(ship["unused_fire_control_stations"], 0)
        self.assertEqual(ship["length_m"], 126.0)
        self.assertEqual(ship["art_path"], "assets/ships/ship-115-midway.webp")
