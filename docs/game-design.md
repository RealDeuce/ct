# Cepheus Trader Game Design and Implementation Overview

This document preserves the long-form game design and implementation context
that formerly lived in the repository README. The README is the project
landing page; [`ROADMAP.md`](../ROADMAP.md) remains authoritative for current
status and milestone order, and the focused documents in this directory remain
authoritative for their individual domains.

Cepheus Trader is a BBS door game in the spirit of *TradeWars 2002*
...
The project is in active alpha development. The local Cepheus Engine rules
reference is the generated Markdown in
[`cepodnew-markdown/`](../cepodnew-markdown/index.md).
...
[`potential-ogc-sources.md`](potential-ogc-sources.md).

The engine is a single authoritative Rust server process. A separate C++20
user-interface process connects to it over a TLS-PSK-protected
...
The common product version is **0.7.13**. The client is
...
[`rpc-and-storage-schema.md`](rpc-and-storage-schema.md).
...
[`player-creation-records.md`](player-creation-records.md).
...
[`field-alpha-operations.md`](field-alpha-operations.md).
...
are defined in
[`stellar-distribution.md`](stellar-distribution.md). It couples
...
[`observed-volume.md`](observed-volume.md).
BBS polity cluster conditioning and gateway topology are specified in
[`bbs-polity-generation.md`](bbs-polity-generation.md).
...
[`celestial-system-generation.md`](celestial-system-generation.md).
...
[`door-presentation.md`](door-presentation.md).
...
[`skill-training.md`](skill-training.md). Initial target selection,
persistent progress, weekly calendar accrual, and course completion are
implemented.

The player-facing command console and the eight phase-independent manager
boundaries are specified in
[`universal-managers.md`](universal-managers.md): crew, ship, task,
message, Known Universe, Operations Ledger, System Common Radio, and Accounts
remain reachable in every operational phase while their individual mutations
stay phase-constrained. The console groups them into Vessel and Crew, Duty and
Accounts, and Information and Communications areas, with presentation and
captain-level utilities in a fourth area. The Known Universe `K` interface is a paged navigation
...
recorded in
[`docked-operations.md`](docked-operations.md). Interplanetary
...
in [`non-interactive-universe-tour.md`](non-interactive-universe-tour.md).
